...
const ConfigSectionBase = 'emtest.cpp';
...
export class Configurations {
  ...
  getDebugConfigurationTemplate(): DebugConfigData {
    const debugConfigData = ((): DebugConfigData => {
      ...
      if (typeof templateFromConfig === 'object' && templateFromConfig !== null) {
        ...
        if (wpLaunchConfigs && Array.isArray(wpLaunchConfigs) && wpLaunchConfigs.length > 0) {
          for (let i = 0; i < wpLaunchConfigs.length; ++i) {
            ...
            this._log.info(
              "using debug config from launch.json. If it doesn't work for you please read the manual: https://github.com/qiuyeyijian/emtest#or-user-can-manually-fill-it",
              template,
            );
        ...
        if (wpLaunchConfigs && Array.isArray(wpLaunchConfigs) && wpLaunchConfigs.length > 0) {
          for (let i = 0; i < wpLaunchConfigs.length; ++i) {
            ...
            this._log.info(
              "using debug config from launch.json by name. If it doesn't work for you please read the manual: https://github.com/qiuyeyijian/emtest#or-user-can-manually-fill-it",
              template,
      ...
      throw Error(
        "For debugging 'emtest.cpp.debug.configTemplate' should be set: https://github.com/qiuyeyijian/emtest#or-user-can-manually-fill-it",
      );
    })();
  ...
  askSentryConsent(): void {
    const envAskSentry = process.env['EMTEST_CPP_ASKSENTRYCONSENT'];
    if (envAskSentry === 'disabled_3') {
      return;
      //const decrypted = Configurations.decrypt(process.env['EMTEST_CPP_LOGSENTRY']);
    ...
    if (logSentry === 'question' || logSentry === 'disable' || logSentry === 'disable_1' || logSentry === 'disable_2') {
      ...
      vscode.window
        .showInformationMessage(
          'Hey there! EMTest has [sentry.io](https://github.com/qiuyeyijian/emtest/blob/master/documents/configuration/log.logSentry.md) integration to ' +
