...
//TODO:release if workspace contains ".vscode/emtest.cpp.json" we have to start loading the tests
export class WorkspaceManager implements vscode.Disposable {
  ...
  async init(forceReload: boolean): Promise<void> {
    ...
    const sbm = vscode.window.setStatusBarMessage('EMTest: loading tests...');
    ...
    try {
      ...
      for (const taskName of runTasks) {
        ...
        if (exitCode !== undefined) {
          if (exitCode !== 0) {
            throw Error(
              `Task "${taskName}" has returned with exitCode(${exitCode}) != 0. (\`emtest.test.advancedExecutables:runTask.${type}\`)`,
            );
          }
        }
      }
    } catch (e) {
      throw Error(`One of the tasks of the \`emtest.test.advancedExecutables:runTask.${type}\` array has failed: ` + e);
    }
  }
  ...
  async _debugInner(test: AbstractTest, cancellation: vscode.CancellationToken, run: vscode.TestRun): Promise<void> {
    try {
      ...
      {
        const setEnvKey = 'emtest.cpp.debug.setEnv';
          ...
          for (const envName in debugConfigData.template[setEnvKey]) {
            ...
            if (typeof envValue !== 'string')
              this._shared.log.warn(
                'Wrong value. emtest.cpp.debug.setEnv should contains only string values',
                envName,
                setEnvKey,
              );
      ...
      if (debugSessionStarted) {
        ...
        throw Error('Failed starting the debug session. Maybe something wrong with "emtest.cpp.debug.configTemplate".');
      }
